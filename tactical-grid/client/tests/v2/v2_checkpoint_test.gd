...
func _initialize() -> void:
	...
	t.check(int(snapshot.get("schema_version", 0)) == 4, "V2 检查点 schema 为 4")
	t.check(snapshot.has("encounter_state") and snapshot.has("hazard_state") and snapshot.has("facility_state"), "schema 4 保存遭遇、危险区和设施状态")
