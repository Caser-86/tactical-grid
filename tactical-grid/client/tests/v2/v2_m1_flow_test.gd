...
func _initialize() -> void:
	...
	t.check(flow.get_guide_text().contains("流程 1/2") and flow.get_guide_text().contains("营救"), "开场持续指示先移动并营救")
	...
	t.check(flow.get_guide_text().contains("流程 2/2") and flow.get_guide_text().contains("绿色撤离区") and flow.get_guide_text().contains("自动完成"), "营救后持续指示进入撤离区自动完成")
