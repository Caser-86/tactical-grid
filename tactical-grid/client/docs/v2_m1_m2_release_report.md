...
本轮完整 V2 门禁报告 `Warnings: 0`。此前的退出清理警告已通过 V2 运行时所有权收口和三个独立 fixture 的释放修复消除；当前仍未完成的是人工验收，不应把自动化门禁通过写成公开发布批准。
...
- 本轮三条路线均在独立 Godot 进程中退出码 0；完整门禁复测未再报告退出清理警告。该重构与 V2 服务、幽灵占位和 fixture 释放收口共同消除了本轮已定位的对象残留来源。
...
4. 不打开 Godot 编辑器，从当前无警告 Windows 候选包完整走通启动、基地、任务选择、失败重试、胜利结算、返回基地和退出。
...
4. 以当前无警告候选包完成逐图人工审查和不打开编辑器的完整 Windows 流程。
