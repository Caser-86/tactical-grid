...
| 检查 | 命令/证据 | 结果 |
|---|---|---|
| V2 总闸门 | `tests/v2/run_v2_gate.ps1` | 74/74 项通过，1633/1633 断言通过 |
...
总闸门报告了 82 条 Godot 退出清理警告，分布在 15 类消息，主要来自部分视觉/场景测试的 `ObjectDB`、`CanvasItem RID` 和资源仍在使用。剩余测试清理警告没有造成断言失败，但仍应在 H3 前完成风险评估或收敛。
...
5. 剩余 82 条测试场景退出警告完成风险评估或修复，并把结论补回本报告。
