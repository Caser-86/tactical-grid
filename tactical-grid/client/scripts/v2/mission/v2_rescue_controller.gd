...
func query_rescue(actor: Unit, rescue_id: StringName) -> Dictionary:
	...
	if _manhattan(actor.grid_pos, target) != 1:
		return {"valid": false, "reason": &"rescue_too_far", "rescue_id": id, "target": target, "distance": _manhattan(actor.grid_pos, target)}
	var preview := {"valid": true, "action": &"rescue", "actor": actor, "actor_id": actor.entity_id, "actor_pos": actor.grid_pos, "rescue_id": id, "character_id": String(entity.get("character_id", "scout")), "target": target, "target_position": target, "cost": {"action": true}, "state_revision": _state_revision}
...
func get_rescue_id_at(cell: Vector2i) -> String:
	for raw_entity in _map_data.get("entities", []):
		if raw_entity is Dictionary:
			...
			if String(entity.get("type", "")) == "objective_primary" and _entity_position(entity) == cell:
...
func get_captive(rescue_id: StringName) -> Dictionary:
	return _get_rescue_entity(String(rescue_id)).duplicate(true)
