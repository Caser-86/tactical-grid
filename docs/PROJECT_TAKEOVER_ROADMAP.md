...
- M1 已有 18×14 单人开场、救出侦察兵、7 名总敌人、最多 3 名同时活跃、上传和撤离流程。
...
- 行动预览与生产执行路径尚未全部统一。
- 迷雾记忆、最后已知位置和敌方公开意图尚未完成运行时表现闭环。
- 地图稳定实体 ID、版本化模式和检查点序列化尚未收口。
- M1 仍需扩展为约 22×16 的三个紧凑遭遇区并通过首次玩家测试。
...
- 行为：
  ...
  - 网络层只在需要时显示连接；节点四状态同时用颜色、形状和动画区分。
...
1. CH1-010 统一行动事务。
2. CH1-020 地图模式、稳定 ID 与检查点。
3. CH1-030 真实输入 E2E 与教学。
4. CH1-040 迷雾运行时闭环。
5. CH1-050 AI 规划与公开意图。
6. CH1-060 网络与警戒表现。
7. CH1-070 至 CH1-090 完成 M1。
8. H1 真人硬门通过后，才进入 R4 和 M2-M6。
