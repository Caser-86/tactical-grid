...
- CH1-010 至 CH1-060 的代码与自动化契约已完成：统一行动、地图 v2/稳定 ID/RNG、真实输入 E2E、移动后即时迷雾刷新、敌方意图、战术网络和警戒表现；真实鼠标点击结束回合、中键拖动镜头和 Space 结束回合均已纳入回归。
...
- Windows 固定 1280×720 布局已关闭 HiDPI 逻辑缩放；在 200% 系统缩放下，正式包窗口实测为 1296×759（含边框），底部 HUD 不再被裁切。
- 旧后端、旧路线图、旧 QA、旧文档归档和无生产引用的 API 客户端已从当前树移除；历史仍可从 Git 提交记录追溯。

### 2.2 尚未完成

- CH1-090 的代码与资源接入已完成第一轮；网络节点不再穿透战争迷雾，玩家移动后会立即刷新新视野，右侧敌方意图摘要已纳入 HUD 位置回归测试，基地角色详情已按职业加载真实单位纹理并纳入 15 项自动契约，对话头像/文本/选项布局已纳入 10 项自动契约，完整流程截图和对话快照已覆盖 1280×720/1920×1080；尚未完成角色/敌军正式方向辨识的人工确认、M1 三区域表现的双分辨率/灰度/色觉人工检查、移动后视野扩展的人工走查和音频听感验收。
...
- 左键选择单位后显示蓝色移动范围与红色攻击范围；点击红色敌人预览攻击、再次点击确认；右键移动/取消、`Tab` 切换单位、`Space` 结束回合、`G` 网络层、`Home` 聚焦。
...
- 状态：代码与自动化验证完成，Windows 发布包窗口尺寸已核验；仍等待 720p/1080p 人工输入验收
- 依赖：CH1-010
- 执行者：Terra high
- 修改：
...
- 行为：
  ...
  - 底部常驻显示左键选择/攻击预览、中键拖动、右键移动/取消、`WASD/方向键`、`Tab`、`G`、`Home` 和 `Space` 的用途，避免首次玩家依赖外部说明。
  - 右键点击友军直接进入移动模式并显示可达范围；移动/目标选择模式下右键取消，`Esc` 只在无目标模式时暂停。
  - 左键点击红色敌人只进入攻击预览，显示命中率、预计伤害和目标 HP；再次点击同一敌人确认攻击，右键取消，移动/攻击按钮不占用主动作栏。
