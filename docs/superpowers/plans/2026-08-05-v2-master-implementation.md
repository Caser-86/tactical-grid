...
| M112 | `Invoke-V2Scene 'tests/v2/v2_m1_e2e_test.tscn'; Invoke-V2Scene 'tests/v2/v2_m1_visual_snapshot.tscn'; powershell -ExecutionPolicy Bypass -File tests/v2/run_m1_visual_matrix.ps1` |
...
- [x] M112 M1 自动 E2E 和视觉矩阵。三路线 60/60；视觉矩阵 42/42（2 分辨率 × 3 模式 × 7 阶段），非空与尺寸门通过。
- [x] M113 首次玩家记录工具。记录器合同 16/16；命令行启用/禁用合同 7/7；BattleController 仅在显式 `--v2-playtest-id=Pxx` 时接入本地匿名记录；普通启动不写试玩文件；首次玩家协议和记录表已建立。
- [ ] H1 三名首次玩家门。
- [ ] M114 H1 修正和 M1 灰盒锁；单击攻击和单位占位修正已自动化验证，H1 真人门仍待执行。
...
- [ ] C01 基地任务选择、四选三和模块配置。
- [x] C02 M2 熄灯协议。正式锁定地图、五段遭遇、两条路线、危险区和结算已接入；M2 视觉矩阵 54/54 通过。
- [x] C03 M2 狙击营救、门和电力。狙击手同关加入、冷却控制室、门/电力设施、撤离反制和检查点已进入正式流程。
- [ ] C04 M3 断轨营救。
- [ ] C05 M3 重装营救、盾卫和轨道。
- [ ] C06 M4 囚笼密钥。
- [ ] C07 M4 协议工程师和牢门组合。
...
## 14. 2026-08-09 审查后执行基线

本节是当前唯一的后续执行队列，不建立第二份 V2 路线图。原 F01-R12 任务定义继续负责具体系统边界；本节负责根据当前真实进度重新排序、补充硬门并给出下一批可派发任务。

### 14.1 已验证事实

- V2 分支和 Godot 用户目录与 V1 隔离，Windows 产品身份为 `Tactical Grid V2: Infiltration`。
- M1、M2 已形成可完成的正式运行流程；M1 为 26×18、12 名敌人、5 个遭遇，M2 为 28×20、13 名敌人、5 个遭遇和 2 组环境危险。
- 当前验收报告记录 V2 总闸门 75/75 项、1640/1640 断言，M1 视觉矩阵 42/42、M2 视觉矩阵 54/54；NX-020 已消除无头音频泄漏，剩余场景树清理仍在进行。
- Windows 包、产品元数据、V2 冷启动和隔离校验已自动通过。
- 三名首次玩家测试、96 张截图逐张审查、包内完整人工流程、音频听感和干净账户验证尚未执行。
- M3-M6 仅有任务数据和批准规格，不存在达到正式关卡标准的锁定地图、完整场景流程和人工验收。
- 当前美术可以支撑功能验证，但地图在迷雾下过暗、单位在实际缩放下轮廓差异不足、工程师/猎手/Boss 尚无完整 V2 运行时美术。
- 无头音频引用已经清除；完整 V2 门当前仍报告 70 条 Godot 退出清理警告（12 类消息）。营救场景的最小复现仍有 10 个 `CanvasItem RID`、24 个 `ObjectDB` 和 2 个脚本资源仍在使用，必须在候选发布前清零或逐项形成可接受的风险结论。
...
```text
M1_M2_STATUS: AUTOMATION_READY / HUMAN_NOT_READY
CHAPTER_STATUS: CONTENT_INCOMPLETE
RELEASE_STATUS: NO_GO
```

自动化通过只能证明已覆盖规则没有发生已知回归，不能证明教学清楚、关卡有趣、时长合理、美术达到正式品质或玩家愿意继续。后续不得再用断言数量、敌人总数、图像数量或任务数据条目替代真人和发布验收。
...
| 代号 | 执行主体 | 负责内容 |
|---|---|---|
| `AI-XH` | 高推理代码/设计模型 | 架构边界、关卡节奏、平衡、复杂缺陷、最终整合审查 |
| `AI-H` | 常规高强度代码模型 | Godot 场景、GDScript、JSON、测试、构建和文档同步 |
| `IMAGE2` | 图像生成工具 | 角色、敌人、环境组件、地标、肖像、图标和特效源图 |
| `HUMAN` | 项目负责人或未参与开发的玩家 | 首次试玩、画面辨识、音频听感、连续通关和发布签核 |

所有代码任务使用测试先行；所有图像任务先做运行时小样并在实际缩放下验收；所有真人门只记录真实证据，AI 不得代签。

### NX-000 当前成果保护与远端同步

**优先级：** P0
**执行主体：** `AI-H`，提交前由 `AI-XH` 复核
**依赖：** 无
**阻塞：** NX-010 及所有后续修改

**文件：**

- 审查当前 `git diff` 中的设置、结算、基地、表现、音频和测试变更。
- 纳入当前已验证的新文件：`scripts/v2/presentation/v2_result_presentation.gd`、设置运行时测试、设置场景测试和结算表现测试。
- 合并生产收口文档中的有效事实到本主计划和 `client/docs/v2_m1_m2_release_report.md`；不保留第二份同职责路线图。

**步骤：**

- [ ] 运行 `git status --short --branch`，确认分支为 `codex/ch1-infiltration-v2`，不处理 V1 工作树。
- [ ] 运行 `git diff --check`，修复空白错误但不改写无关用户内容。
- [ ] 按“设置与存档”“结算与基地”“表现与测试”“文档证据”拆分 2-4 个可审查提交。
- [ ] 每个提交前运行对应局部测试，最后运行 `pwsh -NoProfile -ExecutionPolicy Bypass -File tactical-grid/client/tests/v2/run_v2_gate.ps1`。
- [ ] 推送 `codex/ch1-infiltration-v2`，确认本地不再领先远端且工作树干净。

**验收：** 当前 74 项门禁全部通过；远端拥有可恢复的相同提交；没有构建产物、截图、试玩原始记录或 V1 文件混入提交。
...
- 修改：`docs/v2/README.md`
- 修改：`docs/superpowers/plans/2026-08-05-v2-master-implementation.md`
- 修改：`tactical-grid/client/docs/v2_m1_m2_release_report.md`
- 保留不改：`docs/PROJECT_TAKEOVER_ROADMAP.md`，它只属于 V1 `main`

**步骤：**

- [ ] 把 V2 README 的 M1-only、71/71 和 1592/1592 旧状态更新为 M1/M2 当前事实。
- [ ] 明确 V1 路线图、2026-07-30 规格和 `main` 不是 V2 任务来源。
- [ ] 删除或合并重复的临时收口计划，Git 历史保留其来源。
- [ ] 运行 `rg -n "71/71|1592/1592|M1 graybox playable|M2-M6 仍" docs/v2 tactical-grid/client/docs`，结果不得把旧数字当作当前状态。

**验收：** 开发者从 `docs/v2/README.md` 进入后只会到达本主计划和批准总规格；“已完成、自动通过、真人未验收、不可发布”四种状态不再混写。
...
- 修改：`tactical-grid/client/tests/v2/v2_m1_visual_snapshot.gd`
- 修改：`tactical-grid/client/tests/v2/v2_m2_visual_snapshot.gd`
- 修改：产生泄漏的 V2 场景测试及其清理帮助函数
- 必要时修改：`tactical-grid/client/scripts/game/audio_manager.gd`

**步骤：**

- [ ] 使用 Godot `--verbose` 逐类复现 `ObjectDB`、`CanvasItem RID` 和 `resources still in use`，记录每类来源测试。
- [ ] 先为单一测试补充退出后无资源错误的失败检查，再修复节点、纹理、音频流和场景树释放顺序。
- [ ] 每修复一类后运行其单项场景；不得使用扩大允许列表掩盖泄漏。
- [ ] 运行完整 V2 门并把警告类别、数量和处理结论写回验收报告。

**验收：** 目标为 0 条资源泄漏/仍在使用错误；确属 Godot 4.7.1 引擎已知问题时，必须提供最小复现、影响分析和不影响正式包的证据，不能只写“非致命”。

### NX-030 H1/H1B 试玩候选包冻结

**优先级：** P0
**执行主体：** `AI-H`
**依赖：** NX-000、NX-010；NX-020 可并行但其未完成状态必须写入已知问题

**步骤：**

- [ ] 运行 `pwsh -NoProfile -ExecutionPolicy Bypass -File tactical-grid/client/tools/build_windows.ps1`。
- [ ] 运行 `pwsh -NoProfile -ExecutionPolicy Bypass -File tactical-grid/client/tests/verify_windows_package.ps1`。
- [ ] 运行 `pwsh -NoProfile -ExecutionPolicy Bypass -File tactical-grid/client/tests/v2/v2_release_isolation_test.ps1`。
- [ ] 计算 EXE/PCK SHA-256，写入 `artifacts/v2/verification/h1/BUILD_ID.txt`，该目录不纳入 Git。
- [ ] 项目负责人仅做技术烟雾检查：启动、基地、进入 M1、退出；不得先调优玩法后再测试。

**验收：** 三名玩家使用完全相同的独立 Windows 包；Godot 编辑器关闭；无调试说明、开发者日志或口头教学。

### H1-R2 三名首次玩家 M1/M2 硬门

**优先级：** P0，人类硬门
**执行主体：** `HUMAN`
**依赖：** NX-030
**阻塞：** NX-050 之后的批量美术和所有 M3-M6 生产

**记录：**

- 使用：`tactical-grid/client/docs/v2_first_time_playtest_form.json`
- 汇总：`tactical-grid/client/docs/v2_first_time_playtest_results.md`
- 原始录像和记录：`artifacts/v2/verification/h1/`

**必须记录：**

- [ ] 首次选择、首次有效移动、首次攻击和第一次结束回合所需时间。
- [ ] 是否理解蓝色移动、红色攻击、敌方意图、摄像头、门/电力、危险区和撤离条件。
- [ ] M1/M2 实际时长、回合数、受伤、失能、重试、可选目标和放弃点。
- [ ] 每关最喜欢的决策、最困惑的信息、是否愿意继续下一关。
- [ ] 单位重合、尸体残留、路径越界、迷雾不同步、检查点软锁和任务无法结束是否复现。

**通过条件：** 3/3 能复述主目标和撤离条件；3/3 能独立完成首次攻击；0 个流程阻断；至少 2/3 使用可选内容；至少 2/3 愿意继续。20-25/25-30 分钟目标只在节奏仍有变化且玩家不觉得拖沓时保留，不得靠增加血量或空走凑时长。
...
- [ ] 把每个观察记录归为流程阻断、操作理解、信息层级、节奏、平衡、画面辨识或内容期待。
- [ ] 同一问题至少两名玩家出现时升一级；流程阻断始终为 P0。
- [ ] 每个问题只进入一个后续任务，避免 UI、关卡和美术三个智能体重复修同一现象。
- [ ] 决定维持 20-25/25-30 分钟目标，或基于真实节奏修改总规格；不得在无样本时预先延长。

**验收：** 输出不超过 10 个可执行问题，其中 P0/P1 全部拥有复现、负责任务和验收方式；本任务不直接改代码。

### NX-050 操作、目标和设施可读性修正

**优先级：** P0/P1
**执行主体：** `AI-H`，交互规则由 `AI-XH` 复核
**依赖：** NX-040

**主要文件：**

- 修改：`tactical-grid/client/scripts/ui/tutorial_hint.gd`
- 修改：`tactical-grid/client/scripts/ui/hud.gd`
- 修改：`tactical-grid/client/scripts/v2/presentation/v2_affordance_presenter.gd`
- 修改：`tactical-grid/client/scripts/v2/runtime/v2_battle_controller.gd`
- 测试：对应 `tactical-grid/client/tests/v2/` 输入、HUD、设施和视觉快照测试

**固定方向：**

- [ ] 教学改为短句、贴近当前动作、完成后自动收起，不长期覆盖地图和结束回合按钮。
- [ ] 选中单位立即显示可移动区域与可攻击敌人，颜色之外使用边框、目标牌和路径形状区分。
- [ ] 摄像头操作前显示“将揭露什么”，操作后在地图和目标栏留下已获得情报结果。
- [ ] 当前主目标、下一步位置和撤离条件同时只保留一个最高优先级提示。
- [ ] 所有失败操作返回具体原因和下一步，不显示内部状态名。

**测试：** 先增加失败场景，再运行输入合同、M1/M2 E2E、42/54 视觉矩阵和完整 V2 门。

**验收：** 三名新玩家复测时无需口头解释攻击、设施和撤离；地图操作区域不被持久模态层遮挡。

### NX-060 M1 节奏与内容锁

**优先级：** P1
**执行主体：** `AI-XH` 设计，`AI-H` 实现
**依赖：** NX-050

**主要文件：**

- 修改：`tactical-grid/client/data/v2/missions.json`
- 修改：`tactical-grid/client/data/v2/locked_maps/ch1_m1_echo_yard_v4.json`
- 修改：`tactical-grid/client/data/v2/dialogues.json`
- 测试：M1 内容合同、正式 E2E、视觉矩阵、检查点和基地结算测试

**设计边界：**

- [ ] 保留单人开局、路线分叉、吊机、营救侦察兵和撤离，不新增第二套核心规则。
- [ ] 如果试玩过短，只增加一个有后果的中段决策或一次位置变化，不增加重复敌人波或高血量。
- [ ] 摄像头路线提供可见的情报优势，货柜路线提供可见的距离优势，两条路线都能完成主目标。
- [ ] 营救后让侦察兵立即产生一次角色价值，而不是只增加一个可移动单位。
- [ ] 故事难度保证玩家不因连续固定伤害进入不可恢复死局。

**验收：** 3 名复测玩家中至少 2 名能描述路线差异和侦察兵价值；标准难度没有强制最优路线；检查点恢复不会重复敌人、奖励或设施状态。
...
- 修改：`tactical-grid/client/data/v2/missions.json`
- 修改：`tactical-grid/client/data/v2/locked_maps/ch1_m2_cooling_works_v1.json`
- 修改：M2 任务流、危险区与视觉快照测试

**设计边界：**

- [ ] M2 的主要身份固定为“电力/安全门路线、可预测冷却危险、救出狙击手后的远程解围”。
- [ ] 不重复 M1 的吊桥节奏；中段变化必须来自危险区和路线后果，而不是再次寻找同类终端。
- [ ] 危险区必须提前一整回合用颜色、纹理、图标和文字预告，并提供安全替代路线。
- [ ] 狙击手获救后的第一次远程行动应解决一个此前难处理的威胁，形成明确角色回报。
- [ ] 结算只显示本关实际完成的可选目标、角色和模块。

**验收：** 玩家能清楚说出 M1 与 M2 的不同；至少 2/3 玩家主动利用一次电力/门、危险区或狙击手远程能力；不存在必须读说明才能避开的伤害。

### NX-080 运行时美术辨识小样

**优先级：** P1
**执行主体：** `IMAGE2` 生成，`AI-H` 处理接入，`HUMAN` 视觉签核
**依赖：** H1-R2、NX-050
**阻塞：** NX-090 批量单位美术

**小样范围：** 突击兵、侦察兵、巡逻哨兵、盾卫、协议工程师各 1 套；只制作实际运行时需要的方向和动作状态，不先生产全部角色。

**要求：**

- [ ] 保留现有 2D 正交俯视和工业科幻风格，不切换 3D 或等距视角。
- [ ] 玩家角色使用大面积职业色装甲、不同肩部/背包/武器轮廓；敌人使用不同身体宽高、头部和武器几何。
- [ ] 角色识别不能只依赖红蓝光圈；在 64-96 像素显示、灰度和色觉辅助模式下仍可辨认。
- [ ] 源图、生成日期、工具、处理步骤、最终尺寸、透明背景和用途写入 `data/v2/resource_manifest.md`。
- [ ] 处理后接入真实 M1 场景，生成 720p/1080p、正常/灰度/色觉辅助对比图。

**验收：** 项目负责人在不看名称的情况下对五个小样达到 90% 识别；透明边缘、缩放、遮挡、迷雾和受击/死亡状态无异常。失败时只重做小样，不批量生成。

### NX-090 四角色与七类敌人正式单位批次

**优先级：** P1
**执行主体：** `IMAGE2`、`AI-H`
**依赖：** NX-080 签核

**范围：** 四名玩家、五类通用敌人、猎手、Boss；协议工程师、猎手和 Boss 必须拥有独立 V2 图，不再借用相近单位。

**验收：** 完成 A04、A06、A07、A12 的运行时映射和资源合同；所有单位在实际地图缩放、迷雾、选中、攻击、受击和死亡状态下通过人工辨识。

### NX-100 环境、迷雾与地标表现批次

**优先级：** P1
**执行主体：** `AI-XH` 视觉规划，`IMAGE2` 补图，`AI-H` 接入
**依赖：** NX-080

**范围：**

- [ ] 先调整未探索、已探索、当前可见的明度关系，确保迷雾保留神秘感但不抹掉区域构图。
- [ ] M1 明确货运坪、吊机区、事故记录室、隔离区和撤离区；M2 明确装卸口、涡轮大厅、冷却室、狙击隔离区和撤离区。
- [ ] 可交互设施具备未发现、可操作、已完成和失效四种状态。
- [ ] 复用 V1 四套合法环境组件，只为缺失的任务地标、危险装置和区域识别组件生成新图。

**验收：** 96 张 M1/M2 视觉矩阵逐张人工审查完成；720p、1080p、灰度和色觉模式下目标、危险、单位和地形不互相吞没。

### NX-110 UI、结算、VFX 与音频正式化

**优先级：** P1/P2
**执行主体：** `AI-H`、`IMAGE2`，音频听感由 `HUMAN` 签核
**依赖：** NX-050、NX-090、NX-100

**范围：**

- [ ] 压缩右侧单位面板空白，强化角色身份、生命、行动预算和当前可做动作。
- [ ] 结算页加入任务专属背景/地标、关键行动回顾和明确的下一关动机，保持按钮简洁。
- [ ] 攻击、命中、死亡、设施、营救、危险预警和撤离拥有不同的 VFX/SFX 反馈。
- [ ] 三层音乐变化与警戒状态一致，设置中的音量、减弱动态和关闭震动真实生效。
- [ ] 更新完整资源清单、第三方声明和生成记录。

**验收：** 完成 A08-A13 和 R04 的 M1/M2 范围；无默认图标、无未记录资源、无削波或明显循环接缝；项目负责人完成耳机和扬声器试听。

### H1-LOCK M1/M2 正式垂直切片锁
...
- [ ] 三名首次或低熟练玩家独立完成 M1/M2，0 个流程阻断。
- [ ] 至少 2/3 表示愿意继续 M3。
- [ ] M1/M2 的路线、角色回报和任务身份可以被玩家准确描述。
- [ ] 96 张视觉图人工审查完成，单位与设施辨识通过。
- [ ] Windows 包完整走通启动、基地、任务、失败重试、胜利、返回基地和退出。
- [ ] 自动闸门、包验证和隔离验证全部通过。

未通过时只回到 NX-050 至 NX-110 的对应问题，不得开始 M3 批量生产。

### NX-120 M3 断轨营救与重装兵解锁

**优先级：** P1
**执行主体：** `AI-XH` 设计，`AI-H` 实现，`IMAGE2` 只补独特地标
**依赖：** H1-LOCK

**执行来源：** C01、C04、C05 及 `2026-08-05-v2-p7-p9-missions.md`。

**固定范围：** 三人中选择两人出发；轨道开关是本关唯一主要新概念；关内营救重装兵；重装必须用屏障或占位解决一次明确威胁；完成后基地开放四选三。

**验收：** 正式锁定地图、两条可达路线、五段以内遭遇、失败/检查点/撤离、重装解锁、标准和故事难度、E2E 与视觉矩阵全部完成；3 名玩家中至少 2 名主动使用重装能力。

### NX-130 M4 囚笼密钥

**优先级：** P1
**执行主体：** `AI-XH`、`AI-H`、`IMAGE2`
**依赖：** NX-120

**执行来源：** C06、C07。

**固定范围：** 四选三首次正式使用；摄像头与牢门组合是唯一主要新概念；协议工程师通过夺回设施制造局面变化；可选目标救出技术员但不增加第五名战斗角色。

**验收：** 不依赖固定编队；至少两种编队可标准难度通关；工程师设施行为提前公开；检查点恢复保持门、摄像头和敌方状态一致。
...
**固定范围：** 猎手追踪和信标是本关唯一主要新压力；不新增基础教学弹窗；玩家可通过已学会的摄像头、设施和角色能力反制追踪。

**验收：** 猎手拥有独立轮廓、意图和音效；追踪规则可预测且不瞬间击杀；至少两种反制路线；可选档案只影响图鉴/徽章，不制造复杂经济。

### NX-150 M6 零号终端、Boss 与结局

**优先级：** P1
**执行主体：** `AI-XH` 主导，`AI-H` 实现，`IMAGE2` 完成 Boss/核心地标
**依赖：** NX-140

**执行来源：** C10、C11。

**固定范围：** 两阶段 Boss、护盾终端、核心档案可选目标和明确章节结局；不增加第三阶段、随机词条或章节外新系统。

**验收：** Boss 每阶段意图可读、失败原因可解释；主目标完成后必定进入结局；档案只改变结局文本和图鉴；制作人员与许可证入口可达；从 M5 存档可继续并完成。

### NX-160 全章平衡、连续通关与内容锁

**优先级：** P0
**执行主体：** `AI-XH` 分析，`AI-H` 修复，`HUMAN` 完成 H2
**依赖：** NX-150

**执行来源：** C12-C14、H2。

**验收样本：** 一名玩家从新档连续完成 M1-M6；每关标准难度至少 3 份完成样本、故事难度至少 1 份。记录编队、回合、受伤、失能、能力、设施、可选目标、重试和放弃原因。

**完成条件：** 不存在强制唯一编队、不可恢复血量死局、无限增援、软锁或重复奖励；第一章总时长、角色出场率和模块使用率符合总规格；0 个 P0/P1 内容缺陷。

### NX-170 发布候选与 H3 签核

**优先级：** P0
**执行主体：** `AI-H` 执行 R01-R12，`AI-XH` 复核，`HUMAN` 签核
**依赖：** NX-160、H2

**范围：** 可访问性、损坏存档、视觉矩阵、音频、性能、两小时长时、Windows 身份、许可证、干净克隆、干净账户、缺陷清零、哈希、发布说明和标签。

**最终验收：** 0 崩溃、0 数据损坏、0 流程阻断、0 核心操作失效、0 严重/主要缺陷；V1 工作区、存档和发布物不变；普通玩家无需 Godot 编辑器即可从新档完成 M1-M6 并看到结局。

## 16. 固定执行顺序与并行边界

```text
NX-000 -> NX-010 -> NX-030 -> H1-R2 -> NX-040 -> NX-050
                    NX-020 --------------------------┘

NX-050 -> NX-060 -> NX-070
NX-050 -> NX-080 -> NX-090
NX-080 -> NX-100
NX-060 + NX-070 + NX-090 + NX-100 -> NX-110 -> H1-LOCK

H1-LOCK -> NX-120 -> NX-130 -> NX-140 -> NX-150 -> NX-160 -> NX-170
```

允许并行：

- NX-020 可与试玩包准备并行，但警告状态必须公开记录。
- NX-060 与 NX-080 可在 H1 结果明确后并行，一个只改玩法数据，一个只做已批准小样。
- NX-090 与 NX-100 可在同一小样签核后并行，但不能同时修改同一资源目录或 ArtCatalog 映射。

禁止并行：

- H1-R2 前不得先按开发者猜测重做教学和关卡时长。
- NX-080 未签核前不得批量生成四角色和全部敌人。
- H1-LOCK 未通过前不得开始 M3 正式生产。
- M3-M6 不并行铺关；前一关的新概念必须经过运行和测试后才能成为下一关依赖。
- H2 未通过前不得创建发布候选标签。

## 17. 下一批立即执行项

严格按以下顺序执行，不从美术或 M3 跳开始：

1. NX-000 当前成果保护与远端同步。
2. NX-010 文档权威与状态收口。
3. NX-020 测试退出警告收敛，同时准备 NX-030 试玩候选包。
4. H1-R2 三名首次玩家 M1/M2 无指导试玩。
5. NX-040 根据真实记录决定操作、节奏和时长修正范围。

若暂时无法找到三名首次玩家，可以继续 NX-020、完善试玩材料和做不会改变玩法的构建/文档工作；不得绕过 H1-R2 进入批量美术或 M3。
