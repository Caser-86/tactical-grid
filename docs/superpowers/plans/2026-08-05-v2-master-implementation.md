...
- [ ] F01 V2 发布门和共享测试运行器。
- [ ] F02 V2 数据仓库和模式校验。
- [ ] F03 四角色、敌人、能力、模块和任务基础数据。
- [ ] F04 schema v3 地图加载与校验。
- [ ] F05 V2 存档身份和三槽恢复。
- [ ] F06 一次移动、一次行动和冷却状态。
- [ ] F07 确定性战斗和精确攻击预览。
- [ ] F08 query-preview-validate-commit 行动服务。
- [ ] F09 四角色能力、被动和八模块规则。
- [ ] F10 V2 检查点完整序列化。
- [ ] F11 五类敌人意图与安全后备行为。
- [ ] F12 P1 集成和基础门。
...
- [ ] I04 悬停、锁定、二次确认攻击。
- [ ] I05 具体场景交互菜单。
- [ ] I06 右键取消、Space 结束回合和状态恢复。
- [ ] I07 中键平移、滚轮缩放和 Home 聚焦。
- [ ] I08 UI 方向 3 HUD 和状态提示。
- [ ] I09 精确伤害和命中表现。
- [ ] I10 移动后同事务迷雾刷新。
- [ ] I11 设置、键位、文字缩放和视觉模式。
- [ ] I12 P2 真实输入集成门。
...
- [ ] M101 M1 任务数据与初始编队。
- [ ] M102 22×16 两路线地图和敌人激活区。
- [ ] M103 营救侦察兵并撤离的主目标流。
- [ ] M104 角色营救和同关加入。
- [ ] M105 摄像头和事故记录可选目标。
- [ ] M106 哨兵、无人机和最多三名活跃敌人。
- [ ] M107 M1 潜伏/搜索警戒和即时迷雾。
- [ ] M108 六步行为教学。
- [ ] M109 失败、检查点和三种重试出口。
- [ ] M110 基地解锁、编队、模块和结算。
- [ ] M111 M1 短对话和事故记录。
